...
void main() {
  ...
  test('repairs an already-upgraded v2 manifest missing trust columns', () async {
    final directory = await Directory.systemTemp.createTemp(
      'meshsetu-v2-repair-',
    );
    final file = File('${directory.path}/broken-v2.sqlite');
    final raw = sqlite3.sqlite3.open(file.path);
    raw.execute('''CREATE TABLE site_manifests (
    ...
    raw.execute(
      "INSERT INTO site_manifests VALUES ('site-1', 'Site', 'CODE', NULL, 1, 2, '[]', 3)",
    );
    raw.close();
    ...
    final row = await (migrated.select(
      migrated.siteManifests,
    )..where((table) => table.siteId.equals('site-1'))).getSingle();

    expect(row.authorityKeyId, isNull);
    expect(row.authorityPublicKeyJwk, isNull);
  });
}
