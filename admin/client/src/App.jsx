...
function AuthorityResponseControls({ event, onResponse, response }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true)
    try { await onResponse({ type, messageText: messageText.trim() }) } finally { setSubmitting(false) }
  }
  const states = [
  ...
  return <section className="authority-response-controls"><div className="panel-head"><div><span className="eyebrow">SIGNED RETURN CHANNEL</span><h4>Authority response</h4></div><Badge tone={response?.state === 'RECEIPT_AT_DASHBOARD' ? 'live' : 'warn'}>{response ? response.state : 'NOT SENT'}</Badge></div><div className="response-form"><select value={type} onChange={e => setType(e.target.value)}>{options.map(option => <option key={option.value} value={option.value} disabled={!option.enabled}>{option.label}{option.enabled ? '' : ' — status required'}</option>)}</select><textarea value={messageText} maxLength={256} onChange={e => setMessageText(e.target.value)} placeholder="Keep the verified authority message under 256 UTF-8 bytes." /><button onClick={submit} disabled={submitting || !messageText.trim()}>{submitting ? 'Signing…' : 'Sign and send response'}</button></div><div className="response-timeline">{states.map(([state, label], index) => <span className={`${index <= currentIndex ? 'done' : ''} ${state === response?.state ? 'current' : ''}`} key={state}><i />{label}</span>)}</div><small className="response-honesty">{response?.state === 'RECEIPT_AT_DASHBOARD' ? 'The sender receipt was verified at the control room.' : 'Not delivered yet. This panel does not claim sender delivery until a verified receipt reaches the server.'}</small></section>
}
