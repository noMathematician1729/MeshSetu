...
export type AuthorityResponseRecord = {
  response_id: string; event_id: string; site_id: string; response_type: string;
  message_text: string; destination_ephemeral_id: string; signed_payload: Buffer;
  key_id: string; state: 'SIGNED' | 'GATEWAY_RECEIVED' | 'MESH_QUEUED' | 'FORWARDING' | 'SENDER_DELIVERED' | 'RECEIPT_AT_DASHBOARD' | 'EXPIRED' | 'FAILED';
...
export class EventStore {
  ...
  async init() {
    ...
    await this.pool.query(`CREATE TABLE IF NOT EXISTS authority_responses (response_id text PRIMARY KEY, event_id text NOT NULL REFERENCES sos_incidents(event_id) ON DELETE CASCADE, site_id text NOT NULL, destination_ephemeral_id text NOT NULL, response_type text NOT NULL, message_text text NOT NULL, signed_payload bytea NOT NULL, key_id text NOT NULL, state text NOT NULL, route_mode text, return_hops integer NOT NULL DEFAULT 0, retry_count integer NOT NULL DEFAULT 0, created_at_ms bigint NOT NULL, expires_at_ms bigint NOT NULL, gateway_session_id text, trace_id bytea, original_trace_id bytea)`)
  ...
  async upsert(record: EventRecord) {
    ...
    if (this.pool && (record.origin_ephemeral_id != null || record.ingress_gateway_session_id != null || record.trace_id != null || record.forward_hop_count != null)) {
      await this.pool.query('UPDATE sos_incidents SET origin_ephemeral_id=COALESCE($2,origin_ephemeral_id), ingress_gateway_session_id=COALESCE($3,ingress_gateway_session_id), trace_id=COALESCE($4,trace_id), forward_hop_count=COALESCE($5,forward_hop_count) WHERE event_id=$1', [record.event_id, record.origin_ephemeral_id?.toString?.() ?? null, record.ingress_gateway_session_id ?? null, record.trace_id ? Buffer.from(record.trace_id) : null, record.forward_hop_count ?? null])
    }
    return merged
  }
  async all() {
  ...
  async markGatewayReceived(id: string, gatewaySessionId: string, meshObjectId?: string) {
    if (!this.pool) {
      const row = this.authorityResponses.get(id)
      if (!row) return undefined
      const updated = { ...row, state: 'MESH_QUEUED' as const, gateway_session_id: gatewaySessionId, mesh_object_id: meshObjectId ? Number(meshObjectId) : undefined }
      this.authorityResponses.set(id, updated)
      return updated
    }
    const result = await this.pool.query('UPDATE authority_responses SET state=$2,gateway_session_id=$3 WHERE response_id=$1 AND state IN ($4,$5) RETURNING *', [id, 'MESH_QUEUED', gatewaySessionId, 'SIGNED', 'GATEWAY_RECEIVED'])
    return result.rows[0]
  }

  async updateAuthorityResponse(id: string, patch: Partial<Pick<AuthorityResponseRecord, 'state' | 'route_mode' | 'return_hops' | 'retry_count'>>) {
    if (!this.pool) {
      const current = this.authorityResponses.get(id)
      if (!current) return undefined
    ...
    if (!fields.length) return this.authorityResponse(id)
